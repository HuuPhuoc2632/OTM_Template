...
import 'package:otm_template/components/redacted/redacted_configuration.dart';
...
extension RedactedText on Text {
  Widget redact({RedactedConfiguration? configuration}) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      int maxLines = (this.maxLines ?? 1);
      for (var i = 0; i < maxLines; i++) {
        children.add(
          _RedactedFillWidget(
            configuration: configuration ?? RedactedConfiguration(),
            child: Container(
              margin: EdgeInsets.only(bottom: (maxLines >= 2) ? 2 : 0),
              // ignore: unnecessary_null_comparison
              width: boxes.first == null
                  ? null
                  : (boxes.first.right) / ((i == (maxLines - 1)) ? 2 : 1),
              height: boxes.first.bottom - boxes.first.top,
            ),
          ),
        );
      }
      // for (var box in this.maxLines??1) {
...
class __RedactedFillWidgetState extends State<_RedactedFillWidget> {
  ...
  Widget _buildGlowWidget() {
    var color = widget.configuration.redactedColor ??
        const Color.fromARGB(255, 237, 236, 236).withAlpha(100);
